# coding: utf-8

import os
import sys
import json
import torch
import logging
import pickle

import numpy as np

from tqdm import tqdm
from torch.utils.data import DataLoader, TensorDataset
from transformers import BertTokenizer
from clink.commons import ENT_START_TAG, ENT_END_TAG, ENT_TITLE_TAG


def read_dataset(split_name, data_folder, ratio=1):

    file_name = '%s.jsonl' %split_name
    json_file_path = os.path.join(data_folder, file_name)

    samples = []
    with open(json_file_path, 'rb') as fin:
        for line in fin:
            js = json.loads(line)
            samples.append(js)

    n = len(samples)
    samples = samples[:int(n*ratio)]

    return samples, len(samples)


def read_evaluation_dataset(split_name, data_folder):
    file_name = '%s.jsonl' %split_name
    json_file_path = os.path.join(data_folder, file_name)

    samples = []
    with open(json_file_path, 'rb') as fin:
        for line in fin:
            js = json.loads(line)
            samples.append(js)

    return samples, len(samples)


def read_finetune_dataset(pkl_file_path):
    with open(pkl_file_path, 'rb') as fin:
        all_finetune_data = pickle.load(fin)
        all_context_ids = torch.stack([t['context_id'] for t in all_finetune_data])
        all_entity_index = torch.tensor([t['entity_index'] for t in all_finetune_data], dtype=torch.long)
        all_labels = torch.tensor([t['label'] for t in all_finetune_data], dtype=torch.float)

    return all_context_ids, all_entity_index, all_labels


def select_field(data, key1, key2=None):
    if key2 is None:
        return [example[key1] for example in data]
    else:
        return [example[key1][key2] for example in data]


def get_context_representation(
    sample,
    tokenizer,
    max_seq_length,
    ent_start_token=ENT_START_TAG,
    ent_end_token=ENT_END_TAG,
):
    cls_token = tokenizer.cls_token
    sep_token = tokenizer.sep_token
    context_window = max_seq_length - 4 # reserve place for ent tags, cls and sep

    left_context, right_context, mention_tokens = sample
    mention_tokens = tokenizer.tokenize(mention_tokens)
    context_length = context_window-len(mention_tokens)

    left_context = left_context[-1000:]
    right_context = right_context[:1000]

    left_tokens = tokenizer.tokenize(left_context)
    left_length = min(context_length//2, len(left_tokens))
    right_tokens = tokenizer.tokenize(right_context)
    right_length = min(context_length - left_length, len(right_tokens))
    left_context = left_tokens[-left_length:]
    right_context = right_tokens[:right_length]

    mention_tokens = [ent_start_token] + mention_tokens + [ent_end_token]

    context_tokens = (
        left_context + mention_tokens + right_context
    )
    context_mask = [0]*len(left_context) + [1]*len(mention_tokens) + [0]*len(right_context)

    context_tokens = [cls_token] + context_tokens + [sep_token]
    context_mask = [0] + context_mask + [0]

    input_ids = tokenizer.convert_tokens_to_ids(context_tokens)
    padding = [0] * (max_seq_length - len(input_ids))
    input_ids += padding
    context_mask += padding
    assert len(input_ids) == max_seq_length
    assert len(context_mask) == max_seq_length

    return {
        "tokens": context_tokens,
        "ids": input_ids,
        "mask": context_mask,
    }


def get_candidate_representation(
    sample, 
    tokenizer,
    max_seq_length,
    ent_title_token=ENT_TITLE_TAG
):
    cls_token = tokenizer.cls_token
    sep_token = tokenizer.sep_token

    title_tokens, cand_tokens = sample

    cand_tokens = cand_tokens[:1000]

    title_tokens = tokenizer.tokenize(title_tokens)
    cand_tokens = tokenizer.tokenize(cand_tokens)
    if title_tokens is not None:
        candidate_mask = [1]*len(title_tokens) + [1]*1 + [0]*len(cand_tokens)
        cand_tokens = title_tokens + [ent_title_token] + cand_tokens
    else:
        candidate_mask = [1]*len(cand_tokens)

    cand_tokens = cand_tokens[: max_seq_length - 2]
    cand_tokens = [cls_token] + cand_tokens + [sep_token]
    candidate_mask = candidate_mask[: max_seq_length - 2]
    candidate_mask = [0] + candidate_mask + [0]

    input_ids = tokenizer.convert_tokens_to_ids(cand_tokens)
    padding = [0] * (max_seq_length - len(input_ids))
    input_ids += padding
    candidate_mask += padding
    assert len(input_ids) == max_seq_length
    assert len(candidate_mask) == max_seq_length

    return {
        "tokens": cand_tokens,
        "ids": input_ids,
        "mask": candidate_mask,
    }


def process_mention_data(
    samples,
    tokenizer,
    max_context_length,
    max_cand_length,
    silent,
    debug=False,
    logger=None,
):
    processed_samples = []

    if debug:
        samples = samples[:200]

    if silent:
        iter_ = samples
    else:
        iter_ = tqdm(samples)

    for idx, sample in enumerate(iter_):
        left_context = sample['left_context']
        right_context = sample['right_context']
        mention_tokens = sample['mention']
        name = sample['entity_title']
        description = sample['entity_abstract']
        label = sample.get('label', 0)

        ctxt_cates = sample['context_type']
        ctxt_cate_vec = sample['context_type_ids']
        cand_cates = sample['entity_type']
        cand_cate_vec = sample['entity_type_ids']
        
        context_sample = (left_context, right_context, mention_tokens)
        context_tokens = get_context_representation(
            context_sample,
            tokenizer,
            max_context_length
        )

        candidate_sample = (name, description)
        cand_tokens = get_candidate_representation(
            candidate_sample, 
            tokenizer, 
            max_cand_length
        )

        record = {
            "context": context_tokens,
            "cand": cand_tokens,
            "ctxt_category": ctxt_cate_vec,
            "cand_category": cand_cate_vec,
            "label": label,
        }

        processed_samples.append(record)

    if debug and logger:
        logger.info("====Processed samples: ====")
        for sample in processed_samples[:5]:
            logger.info("Context tokens : " + " ".join(sample["context"]["tokens"]))
            logger.info(
                "Context ids : " + " ".join([str(v) for v in sample["context"]["ids"]])
            )
            logger.info("Candidate tokens : " + " ".join(sample["cand"]["tokens"]))
            logger.info(
                "Candidate ids : " + " ".join([str(v) for v in sample["cand"]["ids"]])
            )

    context_vecs = torch.tensor(
        select_field(processed_samples, "context", "ids"), dtype=torch.long,
    )
    cand_vecs = torch.tensor(
        select_field(processed_samples, "cand", "ids"), dtype=torch.long,
    )
    ctxt_cate_vecs = torch.tensor(
        select_field(processed_samples, "ctxt_category"), dtype=torch.long,
    )
    cand_cate_vecs = torch.tensor(
        select_field(processed_samples, "cand_category"), dtype=torch.long,
    )
    context_mask = torch.tensor(
        select_field(processed_samples, "context", "mask"), dtype=torch.long,
    )
    candidate_mask = torch.tensor(
        select_field(processed_samples, "cand", "mask"), dtype=torch.long,
    )
    labels = torch.tensor(
        select_field(processed_samples, "label"), dtype=torch.long,
    )

    assert context_mask.shape == context_vecs.shape
    tensor_data = TensorDataset(context_vecs, cand_vecs, ctxt_cate_vecs, cand_cate_vecs, context_mask, candidate_mask, labels)
    return None, tensor_data


def process_evaluation_data(
    samples,
    tokenizer,
    max_context_length,
    max_cand_length,
    silent,
    debug=False,
    logger=None,
):
    processed_samples = []

    if debug:
        samples = samples[:200]

    if silent:
        iter_ = samples
    else:
        iter_ = tqdm(samples)

    for idx, sample in enumerate(iter_):
        left_context = sample['left_context']
        right_context = sample['right_context']
        mention_tokens = sample['mention']
        name = sample['entity_title']
        description = sample['entity_abstract']
        label = sample['label']

        ctxt_cates = sample['context_type']
        ctxt_cate_vec = sample['context_type_ids']
        cand_cates = sample['entity_type']
        cand_cate_vec = sample['entity_type_ids']
        
        context_sample = (left_context, right_context, mention_tokens)
        context_tokens = get_context_representation(
            context_sample,
            tokenizer,
            max_context_length
        )

        cand_tokens = {
            "tokens": [],
            "ids": [],
            "mask": [],
        }
        for _name, _description in zip(name, description):
            candidate_sample = (_name, _description)
            _cand_tokens = get_candidate_representation(
                candidate_sample, 
                tokenizer, 
                max_cand_length
            )
            cand_tokens["tokens"].append(_cand_tokens["tokens"])
            cand_tokens["ids"].append(_cand_tokens["ids"])
            cand_tokens["mask"].append(_cand_tokens["mask"])

        record = {
            "context": context_tokens,
            "cand": cand_tokens,
            "ctxt_category": ctxt_cate_vec,
            "cand_category": cand_cate_vec,
            "label": label,
        }

        processed_samples.append(record)

    return processed_samples


def process_multi_data(
    samples,
    tokenizer,
    max_context_length,
    max_cand_length,
    silent,
    debug=False,
    logger=None,
):
    processed_samples = []

    if debug:
        samples = samples[:200]

    if silent:
        iter_ = samples
    else:
        iter_ = tqdm(samples)

    for idx, sample in enumerate(iter_):

        left_context = sample['left_context']
        right_context = sample['right_context']
        mention_tokens = sample['mention']
        name = sample['entity_title']
        description = sample['entity_abstract']
        label = sample['label']

        ctxt_cates = sample['context_type']
        ctxt_cate_vec = sample['context_type_ids']
        cand_cates = sample['entity_type']
        cand_cate_vec = sample['entity_type_ids']
        
        context_sample = (left_context, right_context, mention_tokens)
        context_tokens = get_context_representation(
            context_sample,
            tokenizer,
            max_context_length
        )

        for i, (_name, _description) in enumerate(zip(name, description)):
            candidate_sample = (_name, _description)
            _cand_tokens = get_candidate_representation(
                candidate_sample, 
                tokenizer, 
                max_cand_length
            )

            record = {
                "context": context_tokens,
                "cand": _cand_tokens,
                "ctxt_category": ctxt_cate_vec,
                "cand_category": cand_cate_vec[i],
                "label": label[i],
            }

            processed_samples.append(record)

    if debug and logger:
        logger.info("====Processed samples: ====")
        for sample in processed_samples[:5]:
            logger.info("Context tokens : " + " ".join(sample["context"]["tokens"]))
            logger.info(
                "Context ids : " + " ".join([str(v) for v in sample["context"]["ids"]])
            )
            logger.info("Candidate tokens : " + " ".join(sample["cand"]["tokens"]))
            logger.info(
                "Candidate ids : " + " ".join([str(v) for v in sample["cand"]["ids"]])
            )

    context_vecs = torch.tensor(
        select_field(processed_samples, "context", "ids"), dtype=torch.long,
    )
    cand_vecs = torch.tensor(
        select_field(processed_samples, "cand", "ids"), dtype=torch.long,
    )
    ctxt_cate_vecs = torch.tensor(
        select_field(processed_samples, "ctxt_category"), dtype=torch.long,
    )
    cand_cate_vecs = torch.tensor(
        select_field(processed_samples, "cand_category"), dtype=torch.long,
    )
    context_mask = torch.tensor(
        select_field(processed_samples, "context", "mask"), dtype=torch.long,
    )
    candidate_mask = torch.tensor(
        select_field(processed_samples, "cand", "mask"), dtype=torch.long,
    )
    labels = torch.tensor(
        select_field(processed_samples, "label"), dtype=torch.long,
    )

    # print(context_vecs.shape, cand_vecs.shape, cate_vecs.shape)
    # print(context_mask.shape)
    assert context_mask.shape == context_vecs.shape
    tensor_data = TensorDataset(context_vecs, cand_vecs, ctxt_cate_vecs, cand_cate_vecs, context_mask, candidate_mask, labels)
    return None, tensor_data