...
def evaluate(
    ...
    with torch.no_grad():
        ...
        for step, record in enumerate(iter_):
            ...
            cand_ids = torch.tensor(record["cand"]["ids"], dtype=torch.long).to(device)
            cand_mask = torch.tensor(record["cand"]["mask"], dtype=torch.long).to(device)

            embedding_ctxt, embedding_cands, category_ctxt, category_cand = get_model_obj(ranker).encode(
                context_input=context_ids,
                cand_input=cand_ids,
            )
            
            ctxt_category_labels = [record["ctxt_category"]]
            cand_category_labels = record["cand_category"]
            ...
            for cand in cand_category_label_names:
                nb_entities += 1
