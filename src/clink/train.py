...
from clink.optimizer import get_bert_optimizer
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(iter_):
            ...
            context_input, candidate_input, ctxt_cate_input, cand_cate_input, context_mask, candidate_mask, labels = batch

            semantic_scores, category_scores, category_ctxt, category_cand = get_model_obj(ranker).score_hard_candidates(
                context_input, 
                candidate_input,
            )
            semantic_scores = torch.sigmoid(semantic_scores)

            # print(semantic_scores, category_scores)
...
def get_optimizer(model, params, local_rank=0):
    return get_bert_optimizer(
        [model],
        type_optimization=params["type_optimization"],
...
def main(local_rank, params):
    ...
    if (params['training_objective'] == 'semantic'):
        _lambd = 1
    elif (params['training_objective'] == 'category'):
        _lambd = 0
    else:
        _lambd = None
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(iter_):
            batch = tuple(t.to(device) for t in batch)
            context_input, candidate_input, ctxt_cate_input, cand_cate_input, context_mask, candidate_mask, labels = batch
            if not(args.mention_mask):
                context_mask, candidate_mask=None, None
                
            semantic_loss, category_ctxt_loss, category_cand_loss, final_score = ranker(
                context_input, 
                candidate_input, 
                ctxt_category_input=ctxt_cate_input,
                cand_category_input=cand_cate_input,
            ...
            if (params['training_objective'] == 'semantic'):
                loss = params['sem_loss_weight']*semantic_loss
            elif (params['training_objective'] == 'category'):
                loss = (params['cate_ctxt_loss_weight']*category_ctxt_loss + 
                        params['cate_cand_loss_weight']*category_cand_loss)/(params['cate_ctxt_loss_weight']+params['cate_cand_loss_weight'])
            else:
                loss = (params['sem_loss_weight']*semantic_loss + 
                        params['cate_ctxt_loss_weight']*category_ctxt_loss + 
                        params['cate_cand_loss_weight']*category_cand_loss)/(params['sem_loss_weight']+params['cate_ctxt_loss_weight']+params['cate_cand_loss_weight'])
                ...
                if (logger):
                    logger.info(
                        "Step %d - epoch %d average loss: %.4f; sem loss: %.4f; cate loss: %.4f" %(
                            ...
                            semantic_loss.mean(),
                            (category_ctxt_loss.mean()+category_cand_loss.mean())/2
                        )
                    )
